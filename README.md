...
另外有**狀態轉換圖（State Transition Diagram）**：不用按 GENERATE，編輯左側狀態表時即時把表格畫成泡泡圖 —— 圓圈是狀態、箭頭是轉換條件（Mealy 標 `輸入/輸出`，Moore 把輸出寫在泡泡內）、自我迴圈與雙向轉換自動彎曲避開、初始狀態有起始箭頭、只出現在次態欄的未定義狀態會以紅色虛線提醒，並可下載 SVG。
